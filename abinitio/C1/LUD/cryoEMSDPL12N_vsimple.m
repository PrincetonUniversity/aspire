function [G, W, y, out] = cryoEMSDPL12N_vsimple(NN, G, C, opts)

%------------------------------------------------------------------
% ADMM for Cryo-EM SDP without the spectral norm constraint
%
...
% S = Qtheta(theta,C);
[S, theta]=Qtheta_vsimple(Phi,C,mu);
S=(S+S')/2;
AS = ComputeAX(S);
