function cryo_refine_map_c2(projs_mrc,vol_mrc,map_out_step1,map_out_step2,mat_out,maxiter,tol)
...
sz = 89;
chnk_sz = 500;
stack = imagestackReader(projs_mrc);
projs = zeros(sz,sz,nImages);
perm = randperm(180000,nImages);
perm = sort(perm);
[nr,nc] = size(stack.getImage(1));
projs_chnk = zeros(nr,nc,chnk_sz);
j=1;
msg = [];
for k=1:nImages
    t1 = clock;
    ind = perm(k);
    projs_chnk(:,:,j) = stack.getImage(ind);
    if j == chnk_sz
        projs(:,:,k-chnk_sz+1:k) = cryo_downsample(projs_chnk,sz,true);
        projs_chnk = zeros(nr,nc,chnk_sz);
        j = 1;
    else
        j = j+1;
