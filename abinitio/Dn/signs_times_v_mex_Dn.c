...
inline double sum_norm_3x3(const double *R1, const double *R2) {
...
for (i=start; i<stop; i++) {
	for (j=i+1; j<(N-1); j++) {
		...
		for (k=j+1; k<N; k++) {
            ...
            if (c[4]<best_val) {best_i=0; best_val=c[4];}
			if (c[5]<best_val) {best_i=3; best_val=c[5];}
			if (c[6]<best_val) {best_i=2; best_val=c[6];}
			if (c[7]<best_val) {best_i=3; best_val=c[7];}
