%% Results for O symmetry
% Yoel Shkolnisky, November 2022

[~,s]=ReadMRC('./results_O/volref.mrc');
pixA = s.pixA;
cutoff = 0.5;
...
plotFSCmany(vols,cutoff,pixA,{'1000','1','1/2','1/4'});
figname=fullfile(results_dir,'fsc_O_N50.eps');
print('-depsc',figname);
...
plotFSCmany(vols,cutoff,pixA,{'1000','1','1/2','1/4'});
figname=fullfile(results_dir,'fsc_O_N100.eps');
print('-depsc',figname);
...
plotFSCmany(vols,cutoff,pixA,{'1000','1','1/2','1/4'});
figname=fullfile(results_dir,'fsc_O_N200.eps');
print('-depsc',figname);
...
figname=fullfile(results_dir,'fsc_O_SNR1.eps');
print('-depsc',figname);
...
plotFSCmany(vols,cutoff,pixA,{'25','50','100','200'});
figname=fullfile(results_dir,'fsc_O_SNR2.eps');
print('-depsc',figname);
...
plotFSCmany(vols,cutoff,pixA,{'25','50','100','200'});
figname=fullfile(results_dir,'fsc_O_SNR3.eps');
print('-depsc',figname);
...
plotFSCmany(vols,cutoff,pixA,{'25','50','100','200'});
figname=fullfile(results_dir,'fsc_O_SNR4.eps');
print('-depsc',figname);
...
for snr=1:4
    ...
    for n=1:8
        ...
        axis image
        axis off
        projs_name = fullfile(results_dir,sprintf('noisy_O_n%d_snr%d.eps',n,snr));
...
pixA=0.6495;
results_dir = './results_O';
WriteMRC(vol_ref,pixA,fullfile(results_dir,'volref_10272.mrc'));
vol_relion = ReadMRC('/data/yoelsh/datasets/10272/relion/InitialModel/job122/run_it300_class001.mrc');
[~,~,vol_relion_aligned]=cryo_align_densities(vol_ref,vol_relion,pixA,1,0.5);
WriteMRC(vol_relion_aligned,pixA,fullfile(results_dir,'vol_relion_aligned_10272.mrc'));
vol_aspire = ReadMRC('/data/yoelsh/datasets/10272/abinitio/abinitio.mrc');
[~,~,vol_aspire_aligned]=cryo_align_densities(vol_ref,vol_aspire,pixA,1,0.5);
WriteMRC(vol_aspire_aligned,pixA,fullfile(results_dir,'vol_aspire_aligned_10272.mrc'));

vols={'volref_10272.mrc','vol_aspire_aligned_10272.mrc',...
    'volref_10272.mrc','vol_relion_aligned_10272.mrc'};
...
print('-depsc',fullfile(results_dir,'fsc_10272.eps'));


% figure;
% imagesc(squeeze(sum(GaussFilt(vol_ref,0.1),3)))
% colormap(gray)
% axis image
% axis off
