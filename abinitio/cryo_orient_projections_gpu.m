...
%     % END slower implementatio
    ...
    for j=1:Nrefs        
        ...
        ckjs=sum(bsxfun(@times,candidate_rots_t_vec,clvec2),2);
        ...
        % End of inline implementation of commonline_R.
        Ckj(:,j)=ckj;
        Cjk(:,j)=cjk;
        
        Rtk3=candidate_rots_t_vec(:,3);
        Rtk3=reshape(Rtk3,3,Nrots);
        Rtj3=(Rj(3,:)).';
        idx1=find(sum(bsxfun(@times,Rtk3,Rtj3))<0.999);
        Mkj(idx1,j)=1;
        
        idx2=find(sum(bsxfun(@times,Rtk3,Rtj3))>=0.999);
        Ckj(idx2,j)=-1;
        Cjk(idx2,j)=-1;
        
        [M2kj,C2kj,C2jk]=commonline_R_vec(candidate_rots,Rj,L,0.999);
        assert(all(Ckj(:,j)==C2kj))
        assert(all(Cjk(:,j)==C2jk))
        assert(all(Mkj(:,j)==M2kj))
    ...
    %     assert(norm(Mkj(:)-M2kj(:))==0)
    %     assert(norm(Ckj(:)-C2kj(:))==0)
    %     assert(norm(Cjk(:)-C2jk(:))==0)
