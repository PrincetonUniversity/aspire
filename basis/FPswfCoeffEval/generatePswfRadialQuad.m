function [quadRulePts,quadRuleWts] = generatePswfRadialQuad(n,c,phi_approx_err,lambdaMax)
...
b = phiForQuadweights(x).' * (w.*sqrt(x));

% - Compute matrix A from left hand-side of equation using initial quadrature nodes
A = bsxfun(@times,phiForQuadweights(phiZeros.').',sqrt(phiZeros));
initQuadWeights = A\b;

tol = 1e-16;
objFun = @(quadRule) ( bsxfun(@times,phiForQuadweights(quadRule(1:numQuadPts)).',sqrt(quadRule(1:numQuadPts)).') * quadRule(numQuadPts+1:end) - b);
...
options = optimoptions('lsqnonlin','display','off','MaxIter',1e3,'MaxFunEvals',inf,'TolFun',tol,'TolX',tol);
