function y=E2(deltax,rho,N,idx) 
%
...
% The sum of the squared absolute value of E2, that is,
%   sum(abs(E2(x,rhat(idx),(n-1)./2,idx)).^2)
...
y=(exp(2*pi*1i*(X(idx).*deltax(1)+Y(idx).*deltax(2))/N)-rho);
