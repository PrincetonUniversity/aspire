...
function [debug_data]=genTestGrid(N,eq_filter_angle,s,grid_in)
...
if nargin<4
    ...
    idx=sort(randperm(size(grid_in,3),N));
    rots_grid=grid_in(:,:,idx);
end
...
clearvars eq_min_dist angular_dists %proj_xy proj_xz proj yz
...
debug_data=struct('rots_grid',rots_grid,'grid_stats',grid_stats,'nrot',nrot,...
    'eq_filter_angle',eq_filter_angle,'pol_rep',pol_rep,'q',q,'filtered_idx',filtered_idx);
end