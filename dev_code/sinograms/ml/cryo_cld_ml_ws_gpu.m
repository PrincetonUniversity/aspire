function [S,Z,Zi,PCs,mse,shifts] = cryo_cld_ml_ws_opt(X,L,K,KNN,max_itr,noise_psd,verbose,max_shift,shift_step,Xclean)%% cryo_cld_ml Common lines detection in cryo-em images using EM.
...
for k=1:K
    ...
    gS(:,k)=sum(bsxfun(@times,(gnom1(ind)).',gUnlk.*X(:,nl)),2)/sum(gdenom(ind));
end
S=gather(gS);
%toc
%aaa=1;
