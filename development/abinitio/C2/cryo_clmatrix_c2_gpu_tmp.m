function [clstack,corrstack,shift_equations,shift_equations_map,clstack_mask]=...
    cryo_clmatrix_c2_gpu_tmp(pf,NK,verbose,max_shift,shift_step,min_dist_cls,map_filter_radius,...
...
shifts_1d=zeros(n_proj,n_proj,2);     % Estimated 1D shift between common-lines. 
...
%               g_C((k-1)*n_theta+1:k*n_theta,:)=cryo_average_clmap_gpu(g_C((k-1)*n_theta+1:k*n_theta,:), map_filter_radius);
        ...
        colMask2 = mod(colMask1+n_theta-1,2*n_theta)+1;
        rowMask2 = rowMask1;
        rowMask2(rowMask2<=n_theta) = [];
        rowMask2 = rowMask2 - n_theta;
        
        rowMask1(rowMask1>n_theta)  = [];
        
        mask = ones(n_theta,2*n_theta);
...
%         mask = reshape(mask,n_theta*n_shifts,2*n_theta);
        ...
        g_C = reshape(g_C,[n_theta n_shifts 2*n_theta]);
        
        g_C_  = squeeze(g_C(:,sidx,:));
        g_C_2 = g_C_.*g_mask;
        
        
        [g_sval_2,g_sidx_2] = max(g_C_2(:));

        sval_2 = gather(g_sval_2);
        sidx_2 = gather(g_sidx_2);
        
        [cl1_2,cl2_2]=ind2sub([n_theta, 2*n_theta],sidx_2);
        clstack(k1,k2,2) = cl1_2;
        clstack(k2,k1,2) = cl2_2;
        corrstack(k1,k2,2) = sval_2;
%         shift_2 = -max_shift+(sidx_2-1)*shift_step;
%         shifts_1d(k1,k2,2) = shift_2;
