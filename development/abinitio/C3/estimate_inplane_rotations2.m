...
for i = 1:nImages
    ...
    for j = i+1:nImages
        ...
        corrs = reshape(corrs,n_theta_ij/3,3,nshifts);
        ...
        H(i,j) = cos(3*theta_diff) + sqrt(-1)*sin(3*theta_diff);
...
for i  = 1:nImages
    ...
    c   = real(zi^(1/3));  % we have four time the desired angle
    s   = -imag(zi^(1/3)); % we have four time the desired angle
