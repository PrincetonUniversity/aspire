function [rots,in_plane_rots] = estimate_inplane_rotations3(npf,vis,inplane_rot_res,max_shift,shift_step)
%
% General description
% 
% Input parameters:
...
for i = 1:nImages
    ...
    for j = i+1:nImages
        ...
        inds  = [cijs' cjis'];
        [~,IA,IC] = unique(inds,'rows');   
        inds_unique  = inds(IA,:);
        
        cijs_unique =  inds_unique(:,1);
        cjis_unique =  inds_unique(:,2);
        
        % cross-correaltion - so we want to conjugate        
        Co_unique = bsxfun(@times,Npf_i_shifted(:,cijs_unique,:),conj(Npf_j(:,cjis_unique))); 
        
        Co_unique = sum(Co_unique);
        Co = Co_unique(:,IC,:);
