...
for i = 1:nImages
    ...
    ang_diffs_g = acosd(dot(repmat(qi_gt,1,12),qis));
...
TWELVE = 12; % place-holder for a parameter that would be used for C3 as well
nImages = size(refq,2);
assert(size(rots,3) == nImages);
...
for i=1:nImages 
    ...
    for j=i+1:nImages
        ...
        [~,ind] = min(diffs);
        A_g(i,j) = exp(-sqrt(-1)*2*pi/12*(ind-1));
    end
end
...
angles = exp(sqrt(-1)*2*pi/12*(0:11)).';
