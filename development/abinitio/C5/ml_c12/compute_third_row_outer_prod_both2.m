function [vijs,viis,max_corrs_stats,mse_vij,detec_rate] = ...
    compute_third_row_outer_prod_both2(npf,ciis,cijs,Ris_tilde,R_theta_ijs,max_shift,shift_step,is_handle_equators,refq,is_viz_cls)
...
all_self_corrs = estimate_viis(ciis,Ris_tilde,npf,max_shift,shift_step,shift_phases,is_handle_equators,refq);
...
inds = uint16(sub2ind([n_theta/2,n_theta],cijs(:,:,:,:,1),cijs(:,:,:,:,2)));
clear cijs;
...
%         counter = counter + 1;
        ...
        Corrs = reshape(Corrs,[nRis_tilde,nRis_tilde,n_theta_ij/12,12]);
        
%         Corrs(equators_inds,equators_inds,:,:) = 0;
%         Corrs(equators_inds,:,:,:) = -inf;
%         Corrs(:,equators_inds,:,:) = -inf;
...
%         Corrs = Corrs + bsxfun(@plus,g_all_self_corrs_i,g_all_self_corrs_j.');
        ...
        for s=0:11
            ...
            vijs((i-1)*3+1:i*3,(j-1)*3+1:j*3) = vij/12;
        viis_tmp((i-1)*3+1:i*3,(j-1)*3+1:j*3) = vii/12;
        viis_tmp((j-1)*3+1:j*3,(i-1)*3+1:i*3) = vjj/12;
    ...
    for i=1:nImages
        for j=i+1:nImages
            ...
            rel_rot_angles(i,j) = min([acosd(abs(Ri_gt(:,3).'*Rj_gt(:,3))),...
                ...
                acosd(abs(Ri_gt(:,3).'*g^5*Rj_gt(:,3))),...
                acosd(abs(Ri_gt(:,3).'*g^6*Rj_gt(:,3))),...
                acosd(abs(Ri_gt(:,3).'*g^7*Rj_gt(:,3))),...
                acosd(abs(Ri_gt(:,3).'*g^8*Rj_gt(:,3))),...
                acosd(abs(Ri_gt(:,3).'*g^9*Rj_gt(:,3))),...
                acosd(abs(Ri_gt(:,3).'*g^10*Rj_gt(:,3))),...
                acosd(abs(Ri_gt(:,3).'*g^11*Rj_gt(:,3)))]);
    ...
    for i=1:nImages
        for j=i+1:nImages
            if is_correct(i,j) == 0 && rel_rot_angles(i,j) > 10
                ...
                cijs_found = zeros(1,12);
                cjis_found = zeros(1,12);
                for s=1:12
