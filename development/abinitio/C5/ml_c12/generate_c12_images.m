function [images,refq,ref_shifts] = generate_c12_images(nImages,SNR,projSize,c12_type,max_shift,shift_step)
%
...
%     vol_init_mat = cryo_fetch_emdID(2660);
    ...
    vol = make_vol_c12(masked_vol);
    vol = cryo_mask_volume(vol,25,5);
    assertVolumeIsC12(vol);
%     WriteMRC(vol,1,'ml_c12_orig.mrc');
elseif strcmp(c12_type,'C12')
    vol = cryo_gaussian_phantom_3d('C12_params',projSize,1);
    assertVolumeIsC12(vol);
    WriteMRC(vol,1,'ml_c12_orig.mrc');
elseif strcmp(c12_type,'C1')
    vol = cryo_gaussian_phantom_3d('C1_params',projSize,1);
    vol = make_vol_c12(vol);
    assertVolumeIsC12(vol);
    WriteMRC(vol,1,'ml_c5_init.mrc');
elseif strcmp(c12_type,'C1_Eytan')
    vol = cryo_gaussian_phantom_3d('C1_params',projSize,1);
    vol = make_vol_c12_Eytan(vol);
    assertVolumeIsC12(vol);
else
    error('no such type %s',c12_type);
end
...
function vol_out = make_vol_c12_Eytan(vol_in)
...
vol_out = vol_tmp;
for i=1:11
    vol_out = vol_out + fastrotate3z(vol_tmp,i*360/12);
end
vol_out = vol_out/12;
...
function err = assertVolumeIsC12(vol)
