function [viis,mse_vii] = estimate_viis(ciis,Ris_tilde,npf,max_shift,shift_step,shift_phases,is_handle_equators,refq)
...
inds_scl = sub2ind([n_theta,n_theta/2],ciis(1,:),ciis(2,:));
...
for i=1:nImages
    ...
    Corrs_scl = zeros([numel(inds_scl),1],'gpuArray');
    for s=1:nshifts
        ...
        Corrs_scl_s = PiPi(inds_scl);
        
        Corrs_scl = max([Corrs_scl real(Corrs_scl_s(:))],[],2);
    end
    
    Corrs_scl(cii_equators_inds) = -inf;
    
    [Y,I] = max(real(Corrs_scl));
