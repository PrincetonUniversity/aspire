...
function vol_out = make_vol_c4(vol_in)

vol_out = vol_in;
for i=1:3
    vol_out = vol_out + fastrotate3z(vol_in,i*360/4);
end
vol_out = vol_out/4;
...
function err = assertVolumeIsC4(vol)
   
errs = zeros(1,4);
for i=1:3
    vol_rot = fastrotate3z(vol,90*i);
    errs(i) = norm(vol(:)-vol_rot(:))/norm(vol(:));
end
err = max(errs);
log_message('deviation of volume from c4 symmetry is %.4f',err);
end
