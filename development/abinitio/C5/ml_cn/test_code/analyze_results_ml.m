function [rot_alligned,err_in_degrees,mse] = analyze_results_ml(rots,n_symm,n_theta,refq)

[mse, rot_alligned, sign_g_Ri] = check_rotations_error(rots,n_symm,refq);
log_message('MSE of rotations estimate: %e',mse);

[view_dir_err_in_degrees,polar_ang_gt,stats_g] = check_viewdir_degrees_error(rot_alligned,n_symm,refq);
log_message('view_dir_err_in_degrees: %e',median(view_dir_err_in_degrees));

[err_in_degrees,mean_err_per_rot] = check_degrees_error(rot_alligned,n_symm,n_theta,refq);
...
function [mse, rot_alligned, sign_g_Ri] = check_rotations_error(rotations,n_symm,refq)
...
sign_g_Ri = g_sync(rotations,n_symm,refq);
...
function [view_dir_err_in_degrees,polar_ang_gt,stats_g] = check_viewdir_degrees_error(rots,n_symm,refq)
...
for i = 1:nImages
    ...
    ang_diffs_g = acosd(dot(repmat(qi_gt,1,n_symm),qis));
...
function [err_in_degrees,mean_err_per_rot] = check_degrees_error(rots,n_symm,n_theta,refq)
...
function sign_g_Ri = g_sync(rotations,n_symm,refq)
...
angles = exp(sqrt(-1)*2*pi/n_symm*(0:n_symm-1)).';
