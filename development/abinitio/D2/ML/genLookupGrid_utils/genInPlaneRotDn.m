...
function [inplane_rotated_grid,rots_grid,n_inplane]=genInPlaneRotDn(sphere_grid,dtheta,s)
%% Generate random roatations for testing by chooosing Ri(:,2) randomly
nrot=size(sphere_grid,2);
Ri1=cat(1,-(sphere_grid(2,:)),sphere_grid(1,:),zeros(1,nrot));
Ri1_norms=repmat(sqrt(sum(Ri1.^2,1)),3,1);
Ri1=Ri1./Ri1_norms;
Ri2=cross(Ri1,sphere_grid,1);
