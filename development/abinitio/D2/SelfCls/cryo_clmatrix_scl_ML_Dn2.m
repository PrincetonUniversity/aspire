function [corrs_out]=...
    cryo_clmatrix_scl_ML_Dn2(pf,cl_matrix,max_shift,shift_step,doFilter,scls_data)
%
%
...
%% Run ML in parallel
n=scls_data.n;
M=length(cl_matrix)/(2*n-1);
corrs_out=zeros(M,n_proj,'single');
cl_idx=reshape(cl_matrix,2*n-1,M);
...
for k1=1:n_proj
    ...
    prod_corrs=prod(reshape(g_C(cl_idx),2*n-1,M),1);
    %corrs_out(nonEq_idx,k1)=gather(prod_corrs);
    corrs_out(:,k1)=gather(prod_corrs);
    %2. Non tv equators  
    if mod(n,2)==3 %Neutralized for now
        for eq_idx=1:n_eq
            for i=1:ntheta
                true_scls_corrs=g_C(eq_idx_lists{i,eq_idx,1});   %Only for D_2n
        ...
        for eq_idx=1:n_eq
            for i=1:ntheta             
                ...
                j=nonTv_eq_idx(eq_idx);
                %measures_agg=true_scls_corrs'*eq_measures_i;
                corrs_out((j-1)*ntheta+i,k1)=...
                    corrs_out((j-1)*ntheta+i,k1)*gather(max(eq_measures_i(:))); %DO: Check if this is correct indexing
            end
        end
    end
