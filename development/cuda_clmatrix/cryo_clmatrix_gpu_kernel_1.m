function [clstack,corrstack]=...
    cryo_clmatrix_gpu_kernel_1(pf,NK,verbose,max_shift,shift_step,map_filter_radius)
%
%
...
for k1=1:n_proj
    ...
    for k2=k1+1:n_proj 
        ...
        if norm(proj2(rmax+1,:))>1.0e-13
         ...
         g_C1=gpuArray(zeros(n_theta,n_theta,n_shifts,'single'));
         g_C2=gpuArray(zeros(n_theta,n_theta,n_shifts,'single'));
         cij=gpuArray(zeros(1,'single'));
         cji=gpuArray(zeros(1,'single'));
         maxcorr=gpuArray(zeros(1,'single'));
 
         kernel.ThreadBlockSize = [1, 1, 1];
         kernel.GridSize = [1, 1];
 
         [g_C1,g_C2,cij,cji,maxcorr]=feval(kernel,g_P1,g_P2,rmax,n_theta,...
            max_shift, shift_step,g_C1,g_C2,cij,cji,maxcorr);
    ...
    %g_shift_phases=feval(kernel,max_shift, shift_step, rmax, g_shift_phases);
        ...
        for shiftidx=1:n_shifts
            ...
            err1=norm(C1-g_C1(:,:,shiftidx))/norm(C1);
            if err1>1.0e-5
                aaa=1;
            end
            ...
            err2 = norm(C2-g_C2(:,:,shiftidx))/norm(C2);
            if err2>11.0e-5
                aaa=1;
            end
        ...
        disp([clstack(k1,k2), cij]);
        disp([clstack(k2,k1), cji]);
        t2=clock;
        t=etime(t2,t1);
