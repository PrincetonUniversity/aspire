// Compute the correlations between all rays of two projections, with 
// shifts. This is an improvement over kernel5.cu.
// This function is called by cryo_clmatrix_gpu_kernel_1.mm which is called 
// by test_cryo_clmatrix_gpu_kernel_1.m.        
//
...
/*
__global__ void computePhases(const float max_shift, const float shift_step, const int rmax, float2* shift_phases){
    ...
    for (int shiftidx=0; shiftidx<n_shifts; shiftidx++){
        ...
        float2 c0 = ComplexExp(make_float2(0,-2.0*PI_F*(-rmax)*shift/(2*rmax+1)));
        float2 c1 = ComplexExp(make_float2(0,-2.0*PI_F*shift/(2*rmax+1)));
         for (int j=0; j<rmax; j++){
            //float2 tmp=make_float2(0,-2.0*PI_F*(-rmax+j)*shift/(2*rmax+1));
            shift_phases[rmax*shiftidx+j] = c0;
            c0 = ComplexMul(c0,c1);
         }
    }
}
...
__global__ void commonline(const float2* P1, const float2* P2, 
        ...
        const float shift_step,float *C1, float* C2, 
    ...
    // without allocating an addition array        
        ...
        for (int i = 0; i < n_theta; ++i){
            for (int j = 0; j < n_theta; ++j){
                ...
                corr *= 2.0;
                C1[i+n_theta*j+n_theta*n_theta*shiftidx] = corr;
    ...
    // without allocating an addition array        
        for (int i = 0; i < n_theta; ++i){
            for (int j = 0; j < n_theta; ++j){
                ...
                corr *= 2.0;
                C2[i+n_theta*j+n_theta*n_theta*shiftidx] = corr;
