% Test the function cryo_clmatrix_gpu_kernel_3.m
%
...
%% Odd image, no shifts
K=500;  % Number of projection images 
n=89;   % Each projection image is of size nxn.
max_shift_2d=10;     % The shifts in each projection are between -max_shift_2d  
...
% Find common lines.
tic;
[clstack,corrstack] = cryo_clmatrix_gpu_kernel_3(pf,K,1,max_shift_1d,shift_step_1d);
toc
prop=comparecl( clstack, ref_clstack, n_theta, thetaTol);

tic;
[clstack2,corrstack2] = cryo_clmatrix_gpu(pf,K,1,max_shift_1d,shift_step_1d);
toc
