...
if ~ob.accept1d
	...
	if numel(ob.odim) == 1 && numel(ob.idim) > 1 && ob.size(1) == prod(ob.odim)
		warn '1d odim with full omask and >1d idim may not work if transposed'
	end
end


% embed/select functions (forward)

if isempty(ob.iembed) % [dim(2) *L] -> [idim *L]
	ob.iembed = @(mask, dim, x) fatrix2_embed(mask, dim, x);
end


if isempty(ob.oselect) % [prod(odim) *L] -> [dim(2) *L]
	ob.oselect = @(mask, dim, y) fatrix2_select(mask, dim, y);
end

% embed/select functions (adjoint)

if isempty(ob.oembed) % [dim(1) *L] -> [odim *L] (caution: dim, ?dim swapped)
	ob.oembed = @(mask, dim, y) fatrix2_embed(mask, dim, y);
end

if isempty(ob.iselect) % [dim(1) *L] -> [idim *L] (caution: dim, ?dim swapped)
	ob.iselect = @(mask, dim, x) fatrix2_select(mask, dim, x);
end
