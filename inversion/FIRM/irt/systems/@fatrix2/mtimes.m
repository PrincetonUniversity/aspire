...
fun_forw = @(x) ob.handle_forw(ob.arg, x);

iembed = @(x) ob.iembed(ob.imask, ob.idim, x);
oselect = @(y) ob.oselect(ob.omask, ob.odim, y);
...
if dimx(1) == np % [np (L)] vector mode
	...
	elseif ob.does_many % multiple column vector without loop
		...
		x = reshape(x, [np LL]); % [np *L]
		tmp = iembed(x(:,1)); % [idim]
		tmp = fun_forw(tmp); % [odim]
		y = zeros([prod(ob.odim) LL], class(tmp)); % [*odim *L]
		y(:,1) = tmp(:);
		for ll=2:LL
			tmp = iembed(x(:,ll)); % [idim]
			tmp = fun_forw(tmp); % [odim]
			y(:,ll) = tmp(:);
		end
		y = oselect(y); % [nd *L]
