...
[odims_same odims] = fatrix2_vertcat_odims_same(blocks);
...
% fatrix2_vertcat_odims_same()
% check if odims if each block is same;
% if so, return aggregate odims too
function [odims_same odims] = fatrix2_vertcat_odims_same(blocks)
MM = numel(blocks);
b1 = blocks{1};
odims = cell(MM, 1);
odims_same = true;
