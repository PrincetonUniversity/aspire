...
[nd np] = size(ob);
forw = @(arg, x) nufft_gram_mult(x, arg.ffttoep, arg.mask, np);

if isreal(wi)
	back = forw; % trick: because Hermitian!
else
	warn('T'' not implemented for complex wi')
%	back = []; % do not provide T' if wi is complex
	back = @(arg, x) fail('T'' not implemented for complex wi')
end

arg = ob.arg;
if isempty(arg.mask) % must be fatrix2
	arg.mask = ob.imask_array;
end

[arg.ffttoep arg.reuse] = nufft_gram_init(arg, wi, reuse, nd, np, arg.mask);
...
%
function [ffttoep, reuse] = nufft_gram_init(arg, wi, reuse, nd, np, mask)
...
G = Gnufft('Fatrix', mask, {omega, nufft_args{:}});
