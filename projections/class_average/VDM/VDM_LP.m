...
H=abs(H1); %Diffusion maps weight matrix
M=size(H, 1);
C=sum(H, 2);
...
%eigendecomposition of the VDM matrix and DM matrix
...
%Check if the selected eigenvalues contain negative values
if min(sorted_eigs1)<0
    id=find(abs(sorted_eigs1)>abs(min(sorted_eigs1))); %The selected eigenvalues need to be larger than the largest negative eigenvalues.
    num_eig = length(id); %the total number of selected eigenvectors.
    if num_eig == 0
        fprintf('The top eigenvector has negative eigenvalue, therefore no eigenvector selected. Please check the initial graph structure.\n');
    end
...
t=ceil(log(delta)/log(min(abs(sorted_eigs1(1:num_eig))))); %determine the random walk length min(\lambda)^t \approx delta. 
...
%see the expressions below Eq. (3.16) of the Singer and Wu, Vector Diffusion Maps and Connection Laplacian.   
count=1;
for i=1:num_eig 
    for j=i:num_eig
        list(count, 1)=i;
