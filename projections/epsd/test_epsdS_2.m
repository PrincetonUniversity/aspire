...
%
% Note that since we compute the analytic power spectrum of the noise, the
% noise generation function noise_rexpr is slow.
%
...
[noise,Sfft,Sref,T1]=noise_rexpr(N,max(Ks),1);
...
for j=1:numel(Ks)
    ...
    figname=sprintf('rexpr_%d_%d',N,Ks(j));
    hgsave(figname);
