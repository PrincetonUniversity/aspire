...
% 2. Estimate the isotropic power spectrum of the noise from the corners of
%    the noise images.
...
for j=1:numel(Ks)
    ...
    max_d=floor(N/2);
    [P2,R,~,x]=cryo_epsdS(noise(:,:,1:Ks(j)),samples_idx,max_d,1);
...
for j=1:numel(Ks)
    ...
    figname=sprintf('corners_%d_%d',N,Ks(j));
    hgsave(figname);
