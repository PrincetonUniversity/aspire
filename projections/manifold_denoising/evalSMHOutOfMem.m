function [vCell,dCell] = evalSMHOutOfMem(x,ang_freqs,W_eps,nTheta,maxEigIdx,t_Thr)
N = size(x,2);
...
D = single(zeros(N,1));
save('sGLquan','W','-v7.3');
outOfMemFile = matfile('sGLquan','Writable',true);
...
for j = 1:nChunks
    ...
    outOfMemFile.W(currIdx,:,:) = single(W);
end
