function [vCell,dCell] = evalSMHOutOfMem_v5_sparse(x,ang_freqs,W_eps,nTheta,maxEigIdx,nn)
N = size(x,2);
...
W = sparse(complex(zeros(N)));
% W = sparse(complex(zeros(1e3)));
...
for j = 1:nChunks
    ...
    W = W(:,:,1:fftLen);
    W0_curr = real(W(:,:,1));
    %% Construct symmetric nearest-neighbour pattern matrix
    [~,sortIdx] = sort(W0_curr.','descend');
    nnp_curr = false(N);
    for i=1:numel(currIdx)
        nnp_curr(currIdx(i),sortIdx(1:nn,i)) = 1;
...
%         outOfMemFile{i}.W(currIdx,:) = single(W(:,:,i));
        tmpMatTarget = W(:,:,i);
        tmpMatSource = outOfMemFile{i}.W;
        tmpMatSource(nnp_curr) = tmpMatTarget(nnp_curr(currIdx,:));
        outOfMemFile{i}.W = tmpMatSource;
%         outOfMemFile{i}.W(find(nnp_curr)) = tmpMat(nnp_curr(currIdx,:));
    end
end
