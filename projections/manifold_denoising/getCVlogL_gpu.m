function [ empLkh ] = getCVlogL_gpu( x,ang_freqs,nTheta,trainSize,nv,vCell,dCell,lambda_c)
...
%     [coeff_cv,rank] = denoiseCoeffBySMH(x,ang_freqs,evIdxMat(kVec(k),:),vCell_ext,nv,trainIdx);
    ...
    coeff_cv = gpuArray(single(coeff_cv));
    y = gpuArray(single(y));
    ...
    for i = 1:nChunks
        ...
        Zcurr = gpuArray(zeros(numel(currIdx),numel(trainIdx),maxEmpF+1));
        for m=mIdx
        ...
        minDistCurr = min(min(Zcurr,[],3),[],2);
        Zcurr = bsxfun(@minus,Zcurr,minDistCurr);   % Normalize for better numerics
        Zcurr = exp(-Zcurr/nv/2);
        empLkh = empLkh + sum(log(sum(sum(Zcurr,3),2)) - minDistCurr/nv/2);
    end
end    
empLkh = gather(empLkh);
