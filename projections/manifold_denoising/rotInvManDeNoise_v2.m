function [ projections_man_den,projections_svs_den,projections_sPca] = rotInvManDeNoise_v1( filePath,beta,T,N,nImSMH,maxEigIdx,useSPCA,useWienerFilt,shuffle,normalizeDensity,olN)
%% Add path
...
if useWienerFilt
    x_sMH = PSWF_coeff_denSVS(:,1:nMCV);
    x = PSWF_coeff(:,1:nMCV);
    W_eps = sqrt(2)*8*sqrt(sum(wTot.^4))*nv;
else
    x_sMH = PSWF_coeff(:,1:nMCV);
    x = PSWF_coeff(:,1:nMCV);
...
empLkh = [];
cnt = 1;
[vCell,dCell,~] = evalSMH(x_sMH,ang_freqs,W_eps,nTheta,maxEigIdx,t_Thr,normalizeDensity);
...
while(1)            
    ...
    % - Determine direcion 
    [vCell_inc,dCell_inc,~] = evalSMH(x_sMH,ang_freqs,dEps*W_eps,nTheta,maxEigIdx,t_Thr,normalizeDensity);  
...
%         empLkh_inc = getCVlogL(x,ang_freqs,nTheta,trainSize,nv,vCell_inc,dCell_inc,lambda_c); 
        ...
        dirLambda = 0;
        while(1)            
            empLkh_inc = getCVlogL(x,ang_freqs,nTheta,trainSize,nv,vCell_inc,dCell_inc,lambda_c+dlMCV);
            if empLkh_inc>=empLkh(cnt)
                cnt = cnt + 1;
                empLkh(cnt) = empLkh_inc;
...
[vCell,dCell,denMat] = evalSMH(x_sMH,ang_freqs,W_eps,nTheta,maxEigIdx,t_Thr,normalizeDensity);
...
vCell_ext = smhExtNystrom(PSWF_coeff_denSVS,vCell,dCell,nImSMH,ang_freqs,W_eps,nTheta,normalizeDensity,denMat);
...
trainSize = round(nImages/2);
% kOpt = findCutoffCrossVal(PSWF_coeff,ang_freqs,vCell_ext,evIdxMat,dk,nTheta,nv,trainSize);
lambdaOpt = findCutoffCrossVal(PSWF_coeff,ang_freqs,vCell_ext,evIdxMat,lambda,dl,nTheta,nv,trainSize);
...
[PSWF_coeff_denMan,~] = denoiseCoeffBySMH(PSWF_coeff,ang_freqs,evIdxMat(kOpt,:),vCell_ext,nv,1:nImages);
% [PSWF_coeff_denMan,rankManDen] = denoiseCoeffBySMH(PSWF_coeff,ang_freqs,maxEigIdx,vCell_ext,nv,1:nImages);
% [PSWF_coeff_denMan,rankManDen] = denoiseCoeffBySMH(PSWF_coeff,ang_freqs,maxEigIdx,vCell,nv,1:1e3);
% [PSWF_coeff_denMan,rankManDen] = denoiseCoeffBySMH(PSWF_coeff,ang_freqs,50,vCell,nv,1:1e3);
...
% [ PSWF_coeff_denMan_EM,Likelihood] = denoiseCoeffBySMH_EM( PSWF_coeff,PSWF_coeff_denMan(:,1:nImSMH),ang_freqs,evIdxMat(kOpt,:),vCell_ext,nv,2,nTheta );

%% View preliminary de-noising results for debugging
% testIdx = 10;
% I_denoise_man = Psi(:,ang_freqs==0)*PSWF_coeff_denSVS(ang_freqs==0,testIdx) + 2*real(Psi(:,ang_freqs~=0)*PSWF_coeff_denSVS(ang_freqs~=0,testIdx));
% image_d_disp = zeros(2*L,2*L);
% image_d_disp(r<=r_max) = real(I_denoise_man);
% figure; imagesc(image_d_disp); colormap(gray)

% I_denoise_man = Psi(:,ang_freqs==0)*PSWF_coeff_denMan(ang_freqs==0,testIdx) + 2*real(Psi(:,ang_freqs~=0)*PSWF_coeff_denMan(ang_freqs~=0,testIdx));
% image_d_disp = zeros(2*L,2*L);
% image_d_disp(r<=r_max) = real(I_denoise_man);
% figure; imagesc(image_d_disp); colormap(gray)

% I_denoise_man = Psi(:,ang_freqs==0)*PSWF_coeff_denMan_EM(ang_freqs==0,testIdx) + 2*real(Psi(:,ang_freqs~=0)*PSWF_coeff_denMan_EM(ang_freqs~=0,testIdx));
% image_d_disp = zeros(2*L,2*L);
% image_d_disp(r<=r_max) = real(I_denoise_man);
% figure; imagesc(image_d_disp); colormap(gray)
...
I_denoise_man = mu*ones(1,nImages) + Psi(:,ang_freqs==0)*PSWF_coeff_denMan(ang_freqs==0,:) + 2*real(Psi(:,ang_freqs~=0)*PSWF_coeff_denMan(ang_freqs~=0,:));
...
% I_denoise_man_EM = Psi(:,ang_freqs==0)*PSWF_coeff_denMan_EM(ang_freqs==0,:) + 2*real(Psi(:,ang_freqs~=0)*PSWF_coeff_denMan_EM(ang_freqs~=0,:));
