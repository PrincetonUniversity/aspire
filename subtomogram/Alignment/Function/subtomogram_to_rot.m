function [t, rot_est, rot_s, err, mi] = subtomogram_to_rot(subq, SNR, alpha, ECC)
...
shift_step=1;
if ECC == 1
    [~, corrstack, ~, ~] = commonlines_gaussian_vsub(npf,max_shift,shift_step);
    rec = zeros(1,4);
    for k = 1:n_ang
        rot_s = SO3(:,:,k);
        ind = rot_to_cl(rot_s, tiltq(:,1:ns), n_theta, ns);
        scr = cl_to_score( ind, corrstack, n_theta );
        if scr > rec(2)
            rec(1) = k;
            rec(2) = scr;
        else
    ...
    [~, corrstack, ~, ~] = commonlines_gaussian_vsubeucl(npf,max_shift,shift_step);
    rec = n_r*ns*ones(1,4); 
    for k = 1:n_ang
        rot_s = SO3(:,:,k);
        ind = rot_to_cl(rot_s, tiltq(:,1:ns), n_theta);
        scr = cl_to_score( ind, corrstack, n_theta );
        if scr < rec(2)
            rec(1) = k;
            rec(2) = scr;
...
%disp('time of computing with sampling in SO3');
k = rec(1);
rot_est = SO3(:,:,k);
R1 = q_to_rot(subq(:,1));
R2 = q_to_rot(subq(:,2)); % need to compute relative rotations!!!!!!
rot_s = R2*R1';
err = norm(abs(rot_s-rot_est),'fro');
%disp(err);

diff = zeros(n_ang,1);
for i = 1:n_ang
    diff(i) = norm(abs(SO3(:,:,i) - rot_s),'fro');
end
[mi,~] = min(diff);
