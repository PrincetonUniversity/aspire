function [ clmatrix_i, clmatrix_j ] = getCL_sub_test( b_sub, eul_ij, n_theta, ns )
...
for k = 1:ns
    for l = 1:ns
        ...
        c_ij = round(mod(-theta_ij,2*pi)/(2*pi/n_theta))+1;
        c_ji = round(mod(-theta_ji,2*pi)/(2*pi/n_theta))+1;
%         if c_ij > n_theta/2
...
%         end
        clmatrix_i(k,l) = mod(c_ij-1,n_theta)+1;
        clmatrix_j(l,k) = mod(c_ji-1,n_theta)+1;
    end
end
%     end
% end
