...
%}
function objVal = getObjVal_sub( P , alpha , beta, gamma , K_sub , ns, b_sub, n_theta, numCompThreads )
...
%P = reshape(P,[],n_theta/2*ns,K_sub);

%parpool('local',numCompThreads);
parfor m = 1:siz_so3
    
    [clmatrix_i, clmatrix_j] = getCL_sub( b_sub, [alpha(m), beta(m), gamma(m)], n_theta, ns ); %10/3
    ...
    objVal_ij_sub = squeeze(sum( objVal_ij, 2));
    objVal_ij_rad = squeeze(sum( objVal_ij_sub , 1 ));
