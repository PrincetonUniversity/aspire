...
for ll = 0:maxL
    for i = 1:batch_num 
        ...
        angle = angles(:, startidx+1:startidx+actual_batch_size);
        atemp = reshape(transpose(cell1{ll+1,1}),2*ll+1,1,[]);
        atemp = bsxfun(@times, exp(-1i*(-ll:ll)'*angle(1,:)), atemp);
        atemp = reshape(atemp, 2*ll+1,[]);
        atemp = Wp{1,ll+1}*atemp;
        atemp = reshape(atemp, 2*ll+1, actual_batch_size, []);
        atemp = bsxfun(@times, exp(-1i*(-ll:ll)'*angle(2,:)), atemp);
        atemp = reshape(atemp, 2*ll+1, []);
        atemp = Wm{1,ll+1}*atemp;
        atemp = reshape(atemp, 2*ll+1, actual_batch_size, []);
        atemp = bsxfun(@times, exp(-1i*(-ll:ll)'*angle(3,:)), atemp);
        CR{ll+1,1} = cat(3, CR{ll+1,1}, permute(atemp(ll+1:end,:,:), [3,1,2]));
    end
end
