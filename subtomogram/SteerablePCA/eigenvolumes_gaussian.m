...
load('cleanrib.mat');
R = size(volref,1)/2-1;
F3d = volref(1:end-1,1:end-1,1:end-1);
...
F1 = mvnpdf([omega_all(:,1) omega_all(:,2) omega_all(:,3)],mu1,Sigma1);

threshF = quantile(F1,0.1);
F = scatteredInterpolant(omega_all(:,1),omega_all(:,2),omega_all(:,3),F1);
[xq,yq,zq] = meshgrid(-pi:0.1:pi);
vq = F(xq,yq,zq);
vq2 = mvnpdf([xq(:),yq(:),zq(:)],mu1,Sigma1);
fv = isosurface(xq,yq,zq,vq,threshF);
p = patch(fv);
...
figure()
plot(real(U(:,1))); hold on; plot(real(Us(:,1))); hold on; plot(real(Us(:,2)));
