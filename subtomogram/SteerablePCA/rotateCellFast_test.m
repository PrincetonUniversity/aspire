function [ CR ] = rotateCellFast_test( cell1, angles )
...
for ll = 0:maxL
    for i = 1:N
        Wl = wignerd(ll,angles(:,i)); % from ll to -ll, default uses the - in exponent
        %[T,Tinv] = realY_to_complexY(ll);
        %Wl = real(Tinv*Wl*T);
        AN1{ll+1,1} = cat(3,AN1{ll+1,1},transpose(Wl)*transpose(cell1{ll+1}));%cell1{ll+1}*conj(Wl); % give complex numbers, use real Y?
    end
    AN1{ll+1,1} = permute(AN1{ll+1,1},[2 1 3]);
end
CR = AN1;
